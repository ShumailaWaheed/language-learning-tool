...
class LanguageLearningTool {
    // Properties
    private vocabulary: Map<string, string>; // Map of words and their translations

    // Constructor
    constructor() {
        this.vocabulary = new Map<string, string>();
    }
    ...
    async startQuiz(quizType: string, numberOfQuestions: number): Promise<{ word: string; userTranslation: string; correctTranslation: string; isCorrect: boolean }[]> {
        const quizWords = Array.from(this.vocabulary.entries()).sort(() => Math.random() - 0.5).slice(0, numberOfQuestions);
        const quizResults: { word: string; userTranslation: string; correctTranslation: string; isCorrect: boolean }[] = [];
